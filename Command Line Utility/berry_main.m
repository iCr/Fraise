...
int main (int argc, const char * argv[]) {
	...
	if (!argv[1]) { // There is no argument
		if (![workspace launchApplication:@"Strawberry.app"]) {
			NSLog(@"Can't open Strawberry");
		}
		...
		while (argv[i]) {
			...
			if (![workspace openFile:path withApplication:@"Strawberry.app"]) { // Open file
				NSLog(@"Couldn't open %@ with Strawberry", path);
			}
			i++;
		}
	}
